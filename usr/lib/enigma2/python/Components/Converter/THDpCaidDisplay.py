...
class THDpCaidDisplay(Poll, Converter, object):
					...
					if ecm_info:
						...
						if using:
							if using == "emu":
								...
								textvalue = "(CCcam) %s - %s - %s - %s" % (caid, address, hops, ecm_time)		
						else:
							# mgcamd
							...
							if source:
								if source == "emu":
									...
									textvalue = "(mgcamd) %s - %s - %s" % (caid, source, ecm_time)
							# oscam
							oscsource = ecm_info.get("from", None)
							if oscsource:
								textvalue = "(oscam) %s - %s - %s - %s" % (caid, oscsource, hops, ecm_time)
							# gbox
							...
							if decode:
								if decode == "Internal":
									...
									textvalue = "(gbox) %s - %s" % (caid, decode)
